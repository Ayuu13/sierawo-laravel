...
<x-app-layout>
    ...
    <x-slot name="slot">
        ...
        @php
        $dekorasi = $produklist->where('nama', 'Dekorasi')->where('acara_id', $acaraId)->first();
        @endphp
        ...
        <div class="py-6 flex flex-wrap">
            <div class="py-6 max-w-7xl mx-auto sm:px-10 lg:px-20 space-y-6 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-6">
                    <div>
                        <div>
                            <a class="flex justify-end font-bold text-xl text-black">{{ $dekorasi->acaras->nama }}</a>
                        </div>
                    </div>
                    <div>
                ...
                <div class="flex justify-center">
                    Harga dimulai dari {{ $dekorasi->harga }}
                </div>
                <div class="flex justify-center">
                    {{ $dekorasi->keterangan }}
                </div>
                ...
                <div class="swiper-container">
                    <div class="swiper-wrapper">
                        @foreach ($galeri as $item)
                            @if ($item->produks->nama === 'Dekorasi' && $item->produks->acara_id === $acaraId)
