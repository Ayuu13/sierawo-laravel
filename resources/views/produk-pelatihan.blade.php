...
<x-app-layout>
    ...
    <x-slot name="slot">
        @php
        $acaraIds = [5, 6]; // Daftar acara_id yang ingin diambil
        @endphp
        
        @foreach ($acaraIds as $acaraId)
        
        @php
        $pelatihan = $produklist->where('nama', 'Pelatihan')->where('acara_id', $acaraId)->first();
        @endphp
        ...
        <div class="py-6 flex flex-wrap">
            <div class="py-6 max-w-7xl mx-auto sm:px-10 lg:px-20 space-y-6 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-6">
                    <div>
                        <div>
                            <a class="flex justify-end font-bold text-xl text-black">{{ $pelatihan->acaras->nama }}</a>
                        </div>
                    </div>
                    <div>
                ...
                <div class="flex justify-center">
                    Harga dimulai dari {{ $pelatihan->harga }}
                </div>
                <div class="flex justify-center">
                ...
                <div class="swiper-container">
                    <div class="swiper-wrapper">
                        @foreach ($galeri as $item)
                            @if ($item->produks->nama === 'Pelatihan' && $item->produks->acara_id === $acaraId)
